...
%%%%% 1v 2d  1 l 2 r
figure;
hBar=bar([mean(lrSeg4, 'omitnan'), mean(lrSeg6, 'omitnan'); mean(dvSeg4, 'omitnan'), mean(dvSeg6, 'omitnan')]);
X=cell2mat(get(hBar,'XData')).'+[hBar.XOffset];
hold on  %4 runs
hEB = errorbar(X, [mean(lrSeg4, 'omitnan'), mean(lrSeg6, 'omitnan'); mean(dvSeg4, 'omitnan'), mean(dvSeg6, 'omitnan')], [(std(lrSeg4, 'omitnan')/sqrt(length(lrSeg4))), (std(lrSeg6, 'omitnan')/sqrt(length(lrSeg6))); (std(dvSeg4, 'omitnan')/sqrt(length(dvSeg4))) , (std(dvSeg6, 'omitnan')/sqrt(length(dvSeg6)))], 'vertical', '.', 'Color','black');  % add the errorbar
randVec = (-1 + (1+1)*rand(2,1))/10;
scatter([randVec+X(1,1), randVec+X(1,2), randVec+X(1,3), randVec+X(1,4)], [lrSeg4, lrSeg6],  20, 'k','o','filled'); 
scatter([randVec+X(2,1), randVec+X(2,2), randVec+X(2,3), randVec+X(2,4)], [dvSeg4, dvSeg6],   20, 'k','o','filled'); 
xticks(1:length(1:2)); xticklabels({'L(pre) L(post) R(pre) R(post)','V(pre) V(post) D(pre) D(post)'})
...
%%%%% 1v 2d  1 l 2 r
figure;
hBar=bar([mean(zlrSeg4, 'omitnan'), mean(zlrSeg6, 'omitnan'); mean(zdvSeg4, 'omitnan'), mean(zdvSeg6, 'omitnan')]);
X=cell2mat(get(hBar,'XData')).'+[hBar.XOffset];
hold on  %4 runs
hEB = errorbar(X, [mean(zlrSeg4, 'omitnan'), mean(zlrSeg6, 'omitnan'); mean(zdvSeg4, 'omitnan'), mean(zdvSeg6, 'omitnan')], [(std(zlrSeg4, 'omitnan')/sqrt(length(zlrSeg4))), (std(zlrSeg6, 'omitnan')/sqrt(length(zlrSeg6))); (std(zdvSeg4, 'omitnan')/sqrt(length(zdvSeg4))) , (std(zdvSeg6, 'omitnan')/sqrt(length(zdvSeg6)))], 'vertical', '.', 'Color','black');  % add the errorbar
randVec = (-1 + (1+1)*rand(2,1))/10;
scatter([randVec+X(1,1), randVec+X(1,2), randVec+X(1,3), randVec+X(1,4)], [zlrSeg4, zlrSeg6],  20, 'k','o','filled'); 
